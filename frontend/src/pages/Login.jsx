...
const Login = () => {
  ...
  // Custom color for Login page
  const borderColor = '#2d79f3';
  ...
  const handleGoogleLogin = async () => {
    try {
      ...
      // Verification logic similar to email/pass
      const verifyRes = await fetch(getApiUrl('/api/auth/verify-identity'), {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ firebase_token: token, device_id: navigator.userAgent }),
          ...
          <form className="form" onSubmit={handleSubmit}>
            ...
            <div className="inputForm">
              ...
              <svg
                ...
                style={{ cursor: 'pointer', fill: showPassword ? '#2d79f3' : 'black' }}
              >
...
const StyledWrapper = styled.div`
  .card-wrapper {
    position: relative;
    border-radius: 24px; /* Slightly larger than form */
  ...
  /* Optional: Inner background to hide the center of the gradient if form is transparent, 
     but form has white bg so this is just a backup or for the border effect */
  .card-wrapper::after {
    content: '';
    position: absolute;
    inset: 3px;
  ...
  .form {
    position: relative;
    z-index: 1; /* Above the border effect */
    display: flex;
